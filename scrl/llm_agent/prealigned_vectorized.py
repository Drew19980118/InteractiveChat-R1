...
def compute_vectorized_gt_logprob(
    ...
    Returns:
        ...
        - info_gain_query_eligible: Whether each IG slot has a valid
          target-side score and may participate in semantic query grouping
    ...
    # False preserves a zero-valued IG turn boundary without allowing its
    # query to influence semantic-group statistics downstream.
    info_gain_query_eligible = [[] for _ in range(num_samples)]
    ...
    for turn_idx in range(num_turns):
        ...
        if turn_idx == 0:
            ...
            for global_idx in activate_list:
                if gt_idx[global_idx][0] >= gt_idx[global_idx][1]:
                    # Preserve one zero IG slot for this completed action.
                    info_gain_rewards[global_idx].append(0.0)
                    info_gain_query_eligible[global_idx].append(False)
                    continue
                if global_idx not in gt_values:
                    # No valid step-0 baseline: keep structural alignment.
                    info_gain_rewards[global_idx].append(0.0)
                    info_gain_query_eligible[global_idx].append(False)
                    continue
                ...
                info_gain_query_eligible[global_idx].append(True)
    ...
    result = {
        ...
        'info_gain_query_eligible': info_gain_query_eligible,
