#!/usr/bin/env bash
# Evaluate a selected static ChatR1-style checkpoint on one static benchmark.
set -euo pipefail
...
STATIC_SPLIT_DIR="${STATIC_SPLIT_DIR:-$PROJECT_ROOT/data/static_chatr1_splits/$TRAIN_DATASET}"
...
if (( TRAIN_BATCH_SIZE != 128 || PPO_MINI_BATCH_SIZE != 64 || VAL_BATCH_SIZE != 256 )); then
  echo "ERROR: matched baseline evaluation requires train_batch=128, ppo_mini_batch=64, val_batch=256." >&2
  exit 2
fi
...
echo "[StaticChatR1 Eval] train=$TRAIN_DATASET eval=$EVAL_DATASET checkpoint=$CHECKPOINT_PATH val_batch=256"
...
echo "Completed static ChatR1 evaluation: $EXPERIMENT_NAME"
